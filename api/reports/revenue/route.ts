...
export async function GET(request: NextRequest) {
  try {
    ...
    const serviceRevenueByCategory = serviceRevenue.reduce((acc: Record<string, number>, transaction: any) => {
    ...
    const totalRevenue = revenueBreakdown.reduce((sum: number, item: any) => {
      return sum + (item._sum.amount?.toNumber() || 0)
    }, 0)

    // Format revenue breakdown
    const formattedBreakdown = revenueBreakdown.map((item: any) => {
    ...
    const totalRoomRevenue = roomChargeTransactions.reduce((sum: number, t: any) => sum + t.amount.toNumber(), 0)
    const totalNights = roomChargeTransactions.reduce((sum: number, t: any) => {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          totalTransactions: revenueBreakdown.reduce((sum: number, item: any) => sum + (item._sum.amount ? 1 : 0), 0)
        }
      }
    })
  } catch (error) {
