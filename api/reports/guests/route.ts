...
export async function GET(request: NextRequest) {
  try {
    ...
    const repeatGuests = guestReservationCounts.filter((guest: any) => guest._count.reservations > 1)
    ...
    const averageStayDuration = stayDurations.length > 0
      ? stayDurations.reduce((sum: number, res: any) => sum + res.nights, 0) / stayDurations.length
      : 0
    ...
    return NextResponse.json({
      ...
      data: {
        guestSegments,
        nationalityStats: nationalityStats.map((stat: any) => ({
