...
export async function GET(request: NextRequest) {
  try {
    ...
    const accountsReceivable = invoices.map((invoice: any) => {
    ...
    const summary = {
      ...
      total_outstanding: accountsReceivable.reduce((sum: number, inv: any) => sum + inv.outstanding_balance, 0),
      by_aging: {
        current: 0,
        days_1_30: 0,
        days_31_60: 0,
    ...
    accountsReceivable.forEach((invoice: any) => {
      // Aging buckets
